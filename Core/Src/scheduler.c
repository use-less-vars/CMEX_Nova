...
volatile uint32_t current_time_us = 0;


//changes next_execution-time and sets lies_in_future-flag. No new task is added.
void scheduler_calc_next_execution(Task* task, uint32_t delay_us){
...
void scheduler_run(void) {
    for (uint8_t i = 0; i < MAX_TASKS; i++) {
        if (task_list[i].active && (int32_t)(current_time_us - task_list[i].next_execution) >= 0 && !task_list[i].lies_in_future_time_frame) {
